...
import { ChevronRight, Monitor, Pencil, RefreshCw, Clock } from "lucide-react";
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="font-bold text-white">DREAMLIGHT Live Production Board</h1>
      ...
      <main className="container mx-auto px-4 py-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {columns.map((col) => {
            ...
            const shouldAnimate = col.projects.length >= 3; // Animate if 3 or more projects
              ...
              <div key={col.status} className="rounded-xl p-4 min-h-[400px] bg-slate-900/50 border border-slate-800">
                ...
                {col.projects.length === 0 ? (
                  ...
                  <AnimatedColumn shouldAnimate={shouldAnimate} projectsKey={projectsKey}>
                    {/* Duplicate content untuk infinite scroll effect */}
                    {(shouldAnimate
  ...
  <div className={cn("bg-slate-800 rounded-lg p-3 border transition-all", config.borderColor, "hover:bg-slate-750")}>
    ...
    <div className="space-y-1">
      <div className="flex items-center justify-between text-xs">
        ...
        <span className={cn("font-medium", config.color)}>{getCurrentStageProgress(project)}%</span>
      </div>
    ...
    <div className="flex items-center justify-between mt-2 pt-2 border-t border-slate-700">
      <span className="text-xs text-slate-500">{project.assignedTo}</span>
      {project.status === "kirim" && <span className="text-xs text-amber-400">✓ Sent</span>}
      {project.status === "selesai" && <span className="text-xs text-emerald-400">✓ Done</span>}
    </div>
